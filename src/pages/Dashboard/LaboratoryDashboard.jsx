...
const LaboratoryDashboard = () => {
    ...
    const fetchCounts = async () => {
        try {
            ...
            const clearanceResponse = await axios.get(`http://localhost:8080/Requests/department/${departmentId}/course?courseName=${labCourse}`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });

            const statusCountsResponse = await axios.get(`http://localhost:8080/Status/department/course/${departmentId}/status-counts?courseName=${labCourse}`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
        ...
        <div className={styles.flexContainer}>
            <div className={styles.sidebar}>
                ...
                <nav className={styles.nav}>
                    <button className={styles.whiteButton} onClick={() => navigate('/laboratory-dashboard')}>
                        <img src={dashIcon} alt="Dashboard" className={styles.navIcon} />
                        Dashboard
                    </button>
                    <button className={styles.ghostButton} onClick={() => navigate('/laboratory-clearance-request')}>
